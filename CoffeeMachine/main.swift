...
class CoffeeMachine {
    ...
    var isOn: Bool = false
    var cupsBeforeClean: Int = 5
    var cupsMade: Int = 0
    
    var waterCapacity: Int = 2000
    var coffeeCapacity: Int = 500
    var milkCapacity: Int = 1000
    
    var ingredients: Ingredients = Ingredients(water: 1000, coffee: 300, milk: 1000)
    ...
    var waterStatus: String { "Water: \(ingredients.water)/\(waterCapacity) ml" }
    var coffeeStatus: String { "Coffee: \(ingredients.coffee)/\(coffeeCapacity) g" }
    var milkStatus: String { "Milk: \(ingredients.milk)/\(milkCapacity) ml"}
    var cleaningStatus: String { cupsMade < cupsBeforeClean ? "Needs cleaning after \(cupsBeforeClean - cupsMade) cups" : "Needs cleaning" }
    
    var ingredientsStatus: String { [waterStatus, coffeeStatus, milkStatus].joined(separator: ".") }
    
    func add(_ ingredient: inout Int, amount: Int, maxCapacity: Int, name: String, measurementUnit: String) {
    ...
    var needsCleaning: Bool { cupsMade == cupsBeforeClean }
    ...
    func hasEnoughIngredients(for drink: CoffeeDrink, quantity: Int) -> Bool {
    ...
    func make(drink: CoffeeDrink, quantity: Int) {
        ...
        if hasEnoughIngredients(for: drink, quantity: quantity) {
    ...
    func printRecipe(of drink: CoffeeDrink) {
        ...
        input: while !shouldQuit {
            ...
            case .add:
                ...
                if args.count == 2 {
                    ...
                    guard let amountInt = Int(amountString) else { print("--Incorrect amount"); continue input }
                    amount = amountInt
                }
                else {
                ...
                guard amount > 0 else { print("--Incorrect amount"); continue input }
                switch ingredient {
                case .water:
